...
import { atkBotAction, botCanBring, deckShuffling, defBotAction, defPlayerAction, defineQueue, giveCardToBot, giveCardToPlayer, partEnd, pickCards, reversePlayerMove, tossCardPlayer } from '../store/cardSlice';
import { getCardSign } from '../helpers/functions';
import { emptyTypeCard, fakeCard } from '../helpers/consts';
...
const PlayTablePage: React.FC = () => {
  ...
  const isBotBeat = useSelector((state: RootState) => state.card.isBotBeat);
  const dispatch = useDispatch();

  function canPlayerAddCard(): boolean {
    const isChoose = chooseAtk.length > 0;
    const isPlayer = whoseMove === "player";
    const atkLength = inGame.atk.length < 6;

    function isSuitable(): boolean {
      const res: boolean[] = [];
      if (inGame.atk.length === 0) return true;
      if (inGame.atk.length >= 6) return false;

      for (let i in inGame.atk) {
        for (let j in chooseAtk) {
          if (
            (inGame.def[i] && getCardSign(inGame.def[i].name) === getCardSign(hands.player[chooseAtk[j]].name)) ||
  ...
  useEffect(() => {
    ...
    if (whoseMove === "player" && inGame.atk.length > 0 && isBotBeat) {
      dispatch(defBotAction());
    } else if (whoseMove === "bot" && inGame.atk.length === 0 && isBotBeat) {
    ...
    <div className='table'>
      <div className="container">
        ...
        <div className="table__whoseMove">
          {whoseMove}
          <br />
          {isBotBeat ? "" : <button style={{ background: "red" }} onClick={() => dispatch(botCanBring())}>Пас</button>}
          {/* <button style={{ background: "red" }}>Пас</button> */}
        </div>
        ...
        <div className="table__playground">
          {
            inGame.atk?.map((e, i) => <>
              <div className="table__playground_card">
                <div className="table__playground_card-atk">
                  <Card item={e} isPlayer={false} index={i} isForDef={true} key={i} />
                </div>
                {
                  inGame.def[i] ?
                    <div className="table__playground_card-def">
                      <Card item={inGame.def[i]} isPlayer={false} index={i} isForDef={false} key={i} />
                    </div> : null
                }
              </div >
            </>)
          ...
          {/* <div className="table__playground_card">
            <div className="table__playground_card-atk">
              <Card item={fakeCard} isPlayer={false} index={0} isForDef={false} />
            </div>
            <div className="table__playground_card-def">
              <Card item={fakeCard} isPlayer={false} index={0} isForDef={false} />
            </div>
